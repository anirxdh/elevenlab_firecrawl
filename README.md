...
<p align="center">
  <strong>Your voice controls the browser. Claude AI executes.</strong><br/>
  An autonomous AI browser agent powered by Claude AI + ElevenLabs + Firecrawl that sees your screen, understands context, and takes action — all by voice.
</p>

<p align="center">
  <img src="https://img.shields.io/badge/Claude%20Haiku-4.5-5A67D8?style=flat-square&logo=anthropic" alt="Claude" />
...
| Service | Purpose |
|---------|---------|
| **Claude Haiku 4.5** (Bedrock) | Multimodal reasoning — analyzes screenshots + DOM to decide actions |
...
[ FastAPI Backend ]  ──  POST /task, POST /task/continue, POST /transcribe,
        ...
        |
        v  (AWS SDK / REST APIs)
[ Cloud APIs ]       ──  AWS Bedrock (Claude Haiku), AWS Transcribe,
...
6. Backend sends command + screenshot + DOM + markdown to Claude AI for reasoning
7. Claude returns an action (click, type, navigate, scroll, extract) with a TTS phrase
...
10. Backend re-evaluates with Claude — loops until task is complete (max 25 iterations)
...
- **Multimodal Reasoning**: Claude AI sees screenshots AND reads DOM structure for precise actions
...
- **AWS Account** with Bedrock access (Claude Haiku 4.5 model enabled in us-east-1)
...
```env
# Required — AWS credentials for Claude Haiku 4.5 (Bedrock)
...
> **Important:** Enable Claude Haiku 4.5 model access in your [AWS Bedrock console](https://console.aws.amazon.com/bedrock/) (us-east-1 region).
...
│   ├── services/
│   │   ├── nova_reasoning.py    # Claude Haiku 4.5 via Bedrock (466 lines)
...
| AI Reasoning | Claude Haiku 4.5 (AWS Bedrock) — multimodal vision + language |
...
<p align="center">
  Built for the <strong>ElevenLabs + Firecrawl Hackathon</strong><br/>
  <code>#ElevenLabs</code> &middot; <code>#Firecrawl</code> &middot; <code>#ClaudeAI</code>
</p>
